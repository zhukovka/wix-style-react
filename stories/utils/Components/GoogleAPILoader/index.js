...
export default class GoogleMapsLoader extends React.Component {
  ...
  componentWillMount() {
    if (!window.google) {
      ...
      googleScript.onload = () => this.setState({dependencyStatus: STATUS.success});
  ...
  errorView = () =>
    <div style={{textAlign: 'center'}}>
      <h1>Error :(</h1>
      <div>Excuse me, unable to load required Google Maps API for this component</div>
      <div>Please try again later</div>
    </div>

  successView = () => <div>{this.props.children}</div>
  loadingView = () => <div style={{display: 'flex', justifyContent: 'center'}}><Loader text="Loading Google Maps API..."/></div>
