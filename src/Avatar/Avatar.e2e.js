...
describe('Avatar', () => {
  describe('AutoStory Page', () => {
    const avatarDriver = avatarTestkitFactory({dataHook: storySettings.dataHook});
    const url = createStoryUrl({kind: storySettings.category, story: storySettings.storyName});

    describe('driver sanity', () => {
      it(`should have text content type by default`, async () => {
        await browser.get(url);
        await browser.wait(avatarDriver.exists(), 5000, 'Cannot find <Avatar/>');
  ...
  describe('Test Pages', () => {
    ...
    function testUrl(testCase) {
      return createStoryUrl(
        {
          kind: getTestStoryKind({category: storySettings.category, storyName: storySettings.storyName}),
          story: testCase
        }
        );
    }
