const TextLinkFactory = component => {
  ...
  return {
    ...
    hover: () => browser.actions().mouseMove(component).perform(),
    /* Returns `true` if a prefix icon exists */
    isPrefixIconExists: () => component.$('[data-hook="prefix-icon"]').isPresent(),
    /* Returns `true` if a suffix icon exists */
    isSuffixIconExists: () => component.$('[data-hook="suffix-icon"]').isPresent()
  };
};
