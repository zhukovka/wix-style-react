...
import { tableTestkitFactory, tableTestkitUniFactory } from '../../testkit';
...
describe('Table', () => {
  ...
  describe('[sync]', () => {
    runTests(createRendererWithDriver(TableDriverFactory), tableTestkitFactory);
  });

  describe('[async]', () => {
    runTests(
      createRendererWithUniDriver(tableUniDriverFactory),
      tableTestkitUniFactory,
    );
  });

  function runTests(render, testkitFactory) {
    afterEach(() => cleanup());
    ...
    describe('re-render', () => {
      it('should re-render on data update', async () => {
        ...
        expect(
          await driver.getCell(ROW_INDEX, COLUMN_A_INDEX).textContent,
        ).toBe(newValue);
      });
    ...
    describe('withWrapper', () => {
      ...
      it('should have working test drivers when without wrapper', async () => {
        ...
        const titlebarDriver = testkitFactory({
          wrapper: container,
        ...
        const contentDriver = testkitFactory({
          wrapper: container,
          dataHook: 'test-table-content',
        });
