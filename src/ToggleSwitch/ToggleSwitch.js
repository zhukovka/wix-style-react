...
class ToggleSwitch extends WixComponent {
  ...
  render() {
    const {checked, onChange, size, disabled, colorUnchecked, colorChecked, colorDisabled} = this.props;
    const id = this.id;
        ...
        <label htmlFor={id} className={outerLabel} style={{background: fillColor}} tabIndex={disabled ? null : 0} >
          <label htmlFor={id} className={innerLabel}>
            ...
            <svg className={toggleInactive} viewBox="0 0 143 32">
              <path style={{fill: fillColor}} d="M0 0h142.545v32h-142.545v-32z"/>
            </svg>
          </label>
        </label>
      </div>
...
ToggleSwitch.propTypes = {
  ...
  /** Color for unchecked toggle switch */
  colorUnchecked: PropTypes.string,
  /** Color for checked toggle switch */
